...
import { exists, existsSync } from 'https://deno.land/std@0.102.0/fs/exists.ts';
...
Deno.test('writeJson if not exists', async function (): Promise<void> {
  const notExistsJsonFile = path.join(testdataDir, 'writeJson_file_not_exists.json');

  try {
    assertThrowsAsync(async function (): Promise<void> {
...
Deno.test('writeJson replacer', async function (): Promise<void> {
  ...
  try {
    await writeJson(existsJsonFile, { a: '1', b: '2', c: '3' }, { replacer: ['a'] });
...
Deno.test('writeJsonSync not exists', function (): void {
  const notExistsJsonFile = path.join(testdataDir, 'writeJsonSync_not_exists.json');
...
Deno.test('writeJsonSync if not exists', function (): void {
  const notExistsJsonFile = path.join(testdataDir, 'writeJsonSync_file_not_exists.json');
...
Deno.test('writeJsonSync replacer', function (): void {
  const existsJsonFile = path.join(testdataDir, 'writeJsonSync_replacer.json');

  Deno.writeFileSync(existsJsonFile, new Uint8Array());

  try {
    writeJsonSync(existsJsonFile, { a: '1', b: '2', c: '3' }, { replacer: ['a'] });
